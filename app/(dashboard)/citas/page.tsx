...
export default function CitasPage() {
  ...
  const CardOtro = ({ p, accent }: { p: Paciente; accent?: boolean }) => {
    const score = sc(p);
    const nombre = displayName(p);
    const telC = telContacto(p);
    const telefono = formatTel(p.telefono_encriptado);
      ...
      <div className="dm-card overflow-hidden" style={{ borderColor }}>
        <div className="flex items-center gap-3 p-4">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <button onClick={() => toggleExp(p.id)} className="flex items-center justify-center w-7 h-7 rounded-lg"
        ...
        <div className="space-y-8">
          ...
          {calientes.length > 0 && (
            <section>
              ...
              <div className="space-y-2">
                {calientes.map(p => <CardOtro key={p.id} p={p} accent />)}
              </div>
            </section>
          )}
          ...
          {otros.length > 0 && (
            <section>
              ...
              <div className="space-y-2">
                {otros.map(p => <CardOtro key={p.id} p={p} />)}
              </div>
            </section>
          )}
